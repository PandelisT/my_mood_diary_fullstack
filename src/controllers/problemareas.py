...
def get_problem_area_entries():
    user_id = current_user._get_current_object()
    user = user_id.id
    problem_area_entries = ProblemArea.query.filter_by(
        user_id_fk=user).order_by(ProblemArea.problem_area_date.desc()).all()
    return render_template('problem_area_entries.html',
                           problem_area_entries=problem_area_entries,
                           name=current_user.name)


@problem_area.route("/problem_area_entry/<int:problem_area_id>",
                    methods=["GET"])
@login_required
def get_problem_area_entry(problem_area_id):
    user_id = current_user._get_current_object()
    user = user_id.id
    problem_area_entry = ProblemArea.query.filter_by(
        user_id_fk=user, id=problem_area_id).first()
    return render_template('problem_area_entry.html',
                           problem_area_entry=problem_area_entry)


@problem_area.route("/problem_area_entries/<int:problem_area_id>",
                    methods=["POST"])
@login_required
...
def update_single_problem_area_entry(problem_area_id):
    ...
    db.session.commit()
    return render_template('problem_area_entry.html',
                           problem_area_entry=update_problem_area_entry)
